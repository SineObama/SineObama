...
#define calc(s, d, x) double v##s##d##x = p##d.x - p##s.x
#define len(s, d) double v##s##d = \
    sqrt(v##s##d##x * v##s##d##x + v##s##d##y * v##s##d##y)
#define dot(a, b) (v##a##x * v##b##x + v##a##y * v##b##y)
#define cross(a, b) (v##a##x * v##b##y - v##b##x * v##a##y)
#define angle(a, b, c) double angle##a##b##c = \
    acos(dot(b##a, b##c) / v##b##a / v ##b##c);
#define calc_all(a, b, c) \
    ...
    len(b, c);\
    angle(a, b, c);\
    double cross##a##b##c = cross(b##a, b##c);

    static const double precision = 0.01;
    static const double pi = acos(-1.0);
    calc_all(1, 0, 2);
    if (angle102 < 0 + precision)  // todo 浮点精度问题？
        return false;
    ...
    if (cross102 * cross132 > 0)
        return angle102 > angle132;
    else
        return angle102 + angle132 >= pi;
