...
void newRtStudy(vector<NumTrainData>& trainData) {
    ...
    for (int i = 0; i < testCount; i++) {
        ...
        memcpy(data.data + i * featureLen * sizeof(float), td.data,
...
void newSvmStudy(vector<NumTrainData>& trainData) {
    ...
    for (int i = 0; i < testCount; i++) {
        ...
        normalize(m, m);
        memcpy(data.data + i * featureLen * sizeof(float), m.data,
               featureLen * sizeof(float));
    ...
    param = CvSVMParams(CvSVM::C_SVC, CvSVM::RBF, 10.0, 8.0, 1.0, 10.0, 0.5,
                        0.1, NULL, criteria);
